...
<ul class="nav nav-tabs" style="background-color:#eee; padding:2px;">
			<li class="active">
        <a  href="#1" data-toggle="tab"><i class="fa fa-info-circle" aria-hidden="true"></i> Feed Users</a>
			</li>
  ...
  <div class="form-group">
    ...
    <input type="text" class="form-control" style="padding:15px;" name="name" id="name" placeholder="Name" required="required">
  </div>
  
   <div class="form-group">
    <input type="text" class="form-control" style="padding:15px;" name="surname" id="surname" placeholder="Surname" required="required">
  </div>
  ...
  <p>Choose Profile Picture</p>
    ...
    <input type="file"  style="padding:15px;" name="doc" id="doc" >
  </div>
  
   <div class="form-group">
    <input type="email" class="form-control" style="padding:15px;" name="email" id="email" placeholder="Email" required="required">
  </div>
  
  <div class="form-group">
    <input type="password" class="form-control" style="padding:15px;" name="password" id="password" placeholder="Password" required="required">
  </div>
  <div class="form-group">
    <input type="password" class="form-control" style="padding:15px;" name="cpassword" id="cpassword" placeholder="Confirm password" required="required">
  </div>
  <button type="submit" class="btn btn-info"><i class="fa fa-cloud" aria-hidden="true"></i> Save</button>
          ...
          <table class='table table-striped table-striped'>
            ...
            <th style="background-color:#eee;">Surname</th>
            <th style="background-color:#eee;">Privileges</th>
			<th style="background-color:#eee;">Email</th>
            <th style="background-color:#eee;">Created At</th>
            <th style="background-color:#eee;">Updated At</th>
  ...
  while ($es = mysql_fetch_array ($result)) {
		...
		echo "
			  <tr >
			  <td>{$es['name']}</td><td>{$es['surname']}</td><td>{$b}</td><td>{$es['email']}</td><td>{$es['created_at']}</td><td>{$es['updated_at']}</td>  <td><a href='edit_user.php?id={$es['id']}' > <button type='button' class='btn btn-success btn-xs'><i class='fa fa-pencil' aria-hidden='true'></i> edit </button>  </a></td>   <td><a href='delete_user.php?id={$es['id']}' Onclick='return ConfirmDelete();' > <button type='button' class='btn btn-danger btn-xs'><i class='fa fa-trash' aria-hidden='true'></i> delete </button>  </a></td>
			  </tr>
